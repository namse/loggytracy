...
mod imp {
    ...
    pub fn render() -> String {
        ...
        out.push_str(
            "# HELP loggytracy_memprof_live_bytes Live heap bytes attributed to an arena.\n\
             # TYPE loggytracy_memprof_live_bytes gauge\n",
        );
        for arena in Arena::ALL {
            let index = arena as usize;
            out.push_str(&format!(
                "loggytracy_memprof_live_bytes{{arena=\"{}\"}} {}\n",
                arena.name(),
                LIVE_BYTES[index].load(Ordering::Relaxed)
            ));
        }
        out.push_str(
            "# HELP loggytracy_memprof_peak_bytes High-water live heap bytes for an arena.\n\
             # TYPE loggytracy_memprof_peak_bytes gauge\n",
        );
        for arena in Arena::ALL {
            let index = arena as usize;
            out.push_str(&format!(
                "loggytracy_memprof_peak_bytes{{arena=\"{}\"}} {}\n",
                arena.name(),
                PEAK_BYTES[index].load(Ordering::Relaxed)
            ));
        }
        out.push_str(
            "# HELP loggytracy_memprof_live_allocations Live allocation count for an arena.\n\
             # TYPE loggytracy_memprof_live_allocations gauge\n",
        );
        for arena in Arena::ALL {
            let index = arena as usize;
            out.push_str(&format!(
                "loggytracy_memprof_live_allocations{{arena=\"{}\"}} {}\n",
                arena.name(),
                LIVE_ALLOCS[index].load(Ordering::Relaxed)
            ));
        }
        out.push_str(
            "# HELP loggytracy_memprof_allocated_bytes_total Cumulative bytes allocated by an arena.\n\
             # TYPE loggytracy_memprof_allocated_bytes_total counter\n",
        );
        for arena in Arena::ALL {
            let index = arena as usize;
            out.push_str(&format!(
                "loggytracy_memprof_allocated_bytes_total{{arena=\"{}\"}} {}\n",
                arena.name(),
                TOTAL_BYTES[index].load(Ordering::Relaxed)
            ));
        }
        out.push_str(
            "# HELP loggytracy_memprof_allocations_total Cumulative allocation count for an arena.\n\
             # TYPE loggytracy_memprof_allocations_total counter\n",
        );
        for arena in Arena::ALL {
            let index = arena as usize;
            out.push_str(&format!(
                "loggytracy_memprof_allocations_total{{arena=\"{}\"}} {}\n",
                arena.name(),
                TOTAL_ALLOCS[index].load(Ordering::Relaxed)
            ));
        }
        out.push_str(
            "# HELP loggytracy_memprof_header_bytes Bytes the tag header itself adds.\n\
             # TYPE loggytracy_memprof_header_bytes gauge\n",
        );
        let live_allocations: i64 = Arena::ALL
            ...
            .sum();
        out.push_str(&format!(
            "loggytracy_memprof_header_bytes {}\n",
    ...
    fn mallinfo() -> String {
        ...
        format!(
            "# HELP loggytracy_memprof_malloc_bytes glibc mallinfo2: heap the allocator took from the kernel, split into in-use and retained-free.\n\
             # TYPE loggytracy_memprof_malloc_bytes gauge\n\
             loggytracy_memprof_malloc_bytes{{state=\"arena\"}} {}\n\
             loggytracy_memprof_malloc_bytes{{state=\"mmapped\"}} {}\n\
             loggytracy_memprof_malloc_bytes{{state=\"in_use\"}} {}\n\
             loggytracy_memprof_malloc_bytes{{state=\"free\"}} {}\n\
             loggytracy_memprof_malloc_bytes{{state=\"releasable\"}} {}\n",
