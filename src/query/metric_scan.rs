...
matched {matched} (LOGGYTRACY_MAX_METRIC_SERIES_PER_QUERY) — narrow the selector with more \
attr filters"
            ));
        }
...
series over {} steps is {points} (LOGGYTRACY_MAX_METRIC_POINTS_PER_QUERY) — narrow the \
