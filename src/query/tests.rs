    ...
    async fn the_latency_histogram_is_shaped_for_histogram_quantile() {
        ...
        let bucket = |bound: &str| -> u64 {
            let needle = format!(
                "loggytracy_query_latency_ms_bucket{{endpoint=\"logs\",le=\"{bound}\"}} "
            );
            rendered
                .lines()
        ...
        assert!(
            rendered.contains("loggytracy_query_latency_ms_count{endpoint=\"logs_histogram\"} 1"),
            "volume's own series must carry only volume's observation:\n{rendered}"
        );
        assert!(
            rendered.contains("loggytracy_query_latency_ms_count{endpoint=\"logs\"} 4"),
    ...
    async fn metadata_endpoints_honour_the_requested_time_range() {
        ...
        let values_in = async |start_ns: i64, end_ns: i64| {
            let (status, body) = first_party_get(
                state.clone(),
                &format!(
                    "/loggytracy/api/v1/logs/attributes/app/values?start={start_ns}&end={end_ns}"
                ),
            )
            .await;
            assert_eq!(status, StatusCode::OK, "{body}");
    ...
    async fn a_tenant_over_its_query_quota_is_refused_with_429() {
        ...
        let uri = "/loggytracy/api/v1/logs?attr=app%3Dquota";
