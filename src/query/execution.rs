...
/// Runs a scan whose sink folds the rows instead of collecting them.
///
/// The general metric path's counterpart to `run_metric_count_scan`. The
/// evaluator never wanted the rows — its first act was to reduce them to a
/// `(timestamp, value)` per label set and drop every line — so the
/// `Vec<(SharedLabels, LogEntry)>` in between existed only to be discarded.
/// That intermediate is what `max_metric_rows` used to bound, which is why a
/// `rate()` over a busy stream came back refused rather than answered.
///
/// The fold goes through the same scan as every other read path rather than
/// beside it, because that is where the deletion mask lives and a second scan
/// would be a second place to forget it.
#[allow(clippy::too_many_arguments)]
async fn run_metric_event_scan(
    state: Arc<AppState>,
    tenant: TenantId,
    parsed: logql::LogQuery,
    range: part::QueryTimeRange,
    ...
    runtime_override: Option<std::time::Duration>,
    columns: part::ColumnSet,
    mut sink: MetricEventSink,
) -> Result<(MetricEventSink, u64, u64), String> {
    let max_runtime = runtime_override.unwrap_or(state.config.max_query_runtime);
    let part_guard = tokio::time::timeout(
        max_runtime,
        pin_query_parts(&state, &tenant, &parsed, range),
    ...
    let mut task = tokio::task::spawn_blocking(move || {
        ...
        let (scanned_rows, scanned_bytes) =
            scan.run_into(&state.memtable, &state.parts, &mut sink)?;
        // The sink counts them itself: it owns the mask, so there is no
        // borrowed counter to reach through from inside the scan.
        state
            .delete_requests
            .metrics
            .hidden_rows
            .fetch_add(sink.hidden_rows, Ordering::Relaxed);
        Ok::<_, String>((sink, scanned_rows, scanned_bytes))
    });
