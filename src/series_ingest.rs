...
mod tests {
    ...
    async fn export_acknowledges_after_journal_append_and_replay_reproduces_it() {
        let config = Config {
            data_dir: std::env::temp_dir()
                .join(format!("loggytracy-metric-ingest-{}", uuid::Uuid::new_v4())),
            ..Config::default()
        };
    ...
    async fn a_delta_stream_replays_into_the_same_running_totals() {
        let config = Config {
            data_dir: std::env::temp_dir()
                .join(format!("loggytracy-metric-delta-{}", uuid::Uuid::new_v4())),
            ..Config::default()
        };
        let (service, series_memtable, journal) = service_over(config);
        let base = now_ns();
    ...
    async fn export_rejects_while_draining_without_inserting() {
        let config = Config {
            data_dir: std::env::temp_dir()
                .join(format!("loggytracy-metric-drain-{}", uuid::Uuid::new_v4())),
            ..Config::default()
        };
    ...
    async fn export_is_refused_once_the_buffers_are_over_their_limit() {
        let config = Config {
            data_dir: std::env::temp_dir()
                .join(format!("loggytracy-metric-gate-{}", uuid::Uuid::new_v4())),
            flush_max_bytes: 1,
    ...
    async fn export_rejects_a_request_past_the_decomposed_sample_cap() {
        let config = Config {
            data_dir: std::env::temp_dir()
                .join(format!("loggytracy-metric-cap-{}", uuid::Uuid::new_v4())),
            ..Config::default()
        };
        let (service, series_memtable, _journal) = service_over(config);
    ...
    async fn export_rejects_a_timestamp_outside_the_window() {
        let config = Config {
            data_dir: std::env::temp_dir()
                .join(format!("loggytracy-metric-window-{}", uuid::Uuid::new_v4())),
            ..Config::default()
        };
        let (service, series_memtable, _journal) = service_over(config);
        let request = request_with(
