...
impl FaultConfig {
    /// Reads the `LOGGYTRACY_OBJECT_STORE_*` load knobs. Returns `None` when
    ...
    fn from_vars(vars: Vec<(String, String)>) -> Result<Option<Self>, String> {
        ...
        let write_ms = parse_u64(
            "LOGGYTRACY_OBJECT_STORE_LATENCY_MS",
            lookup("LOGGYTRACY_OBJECT_STORE_LATENCY_MS"),
        )?;
        let read_ms = parse_u64(
            "LOGGYTRACY_OBJECT_STORE_READ_LATENCY_MS",
            lookup("LOGGYTRACY_OBJECT_STORE_READ_LATENCY_MS"),
        )?;
        let jitter_ms = parse_u64(
            "LOGGYTRACY_OBJECT_STORE_LATENCY_JITTER_MS",
            lookup("LOGGYTRACY_OBJECT_STORE_LATENCY_JITTER_MS"),
        )?;
        let error_rate = parse_f64(
            "LOGGYTRACY_OBJECT_STORE_ERROR_RATE",
            lookup("LOGGYTRACY_OBJECT_STORE_ERROR_RATE"),
        )?;
        let seed = parse_u64(
            "LOGGYTRACY_OBJECT_STORE_FAULT_SEED",
            lookup("LOGGYTRACY_OBJECT_STORE_FAULT_SEED"),
        )?;
        ...
        if !(0.0..=1.0).contains(&error_rate) {
            return Err(
                "LOGGYTRACY_OBJECT_STORE_ERROR_RATE must be between 0.0 and 1.0".to_string(),
            );
        }
...
mod tests {
    ...
    fn read_latency_defaults_to_write_latency() {
        let config = FaultConfig::from_vars(vec![(
            "LOGGYTRACY_OBJECT_STORE_LATENCY_MS".to_string(),
            "20".to_string(),
        )])
        .unwrap()
        .unwrap();
    ...
    fn rejects_out_of_range_error_rate() {
        let error = FaultConfig::from_vars(vec![(
            "LOGGYTRACY_OBJECT_STORE_ERROR_RATE".to_string(),
            "1.5".to_string(),
        )])
        .unwrap_err();
