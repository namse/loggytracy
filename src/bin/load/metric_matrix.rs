...
//! loggytracy paths below are the read surface Phase 7 must implement, not a
            ...
            MetricShape::InstantAlert => {
                for (index, service) in services.iter().enumerate() {
                    for window in 0..windows {
                        ...
                        let (description, path) = match cfg.target {
                            Target::Loggytracy => first_party_instant(
                                counter,
                                &[
                                    ("attr", format!("service={service}")),
                                    ("func", "rate".to_string()),
...
(rate({HISTOGRAM_NAME}_bucket{{service=\"{service}\"}}[{range}])))"
                        ...
                        let (description, path) = match cfg.target {
                            Target::Loggytracy => first_party_range_at(
...
fn first_party_instant(metric: &str, params: &[(&str, String)], at_ns: i64) -> (String, String) {
    ...
    let query = encoded.finish();
    let path = format!("/loggytracy/api/v1/metrics/instant?{query}");
    (path.clone(), path)
}
...
pub fn digest_metric_for(target: Target, body: &[u8]) -> Result<MetricAnswer, String> {
    match target {
        Target::Loggytracy => digest_first_party_metric_response(body),
...
mod tests {
    ...
    fn every_first_party_metric_path_is_a_flat_query_under_the_metrics_routes() {
        let queries = build_metric_queries(&config_for(Target::Loggytracy));
        assert!(!queries.is_empty());
        for query in &queries {
            assert!(
                query.path.starts_with("/loggytracy/api/v1/metrics/"),
                "{}",
                query.path
            );
        ...
        let quantiles: Vec<&MetricQuery> = queries
            ...
            .collect();
        assert!(
            quantiles.iter().all(|query| query
                .path
                .starts_with("/loggytracy/api/v1/metrics/quantile?")),
