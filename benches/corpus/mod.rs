//! Deterministic corpus for the benchmarks.
//!
...
use loggytracy::memtable::{Labels, LogEntry, MemTableSnapshot, TenantStreams};
use loggytracy::part::Row;
...
const STATUSES: [u32; 6] = [200, 200, 204, 400, 404, 500];
...
pub fn generate(spec: &CorpusSpec) -> Corpus {
    ...
    let tenant_ids: Vec<TenantId> = (0..tenant_count)
        .map(|index| {
            TenantId::parse(&format!("bench-tenant-{index:03}")).expect("bench tenant id is valid")
        })
        .collect();
