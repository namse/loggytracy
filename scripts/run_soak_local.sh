...
BIN="${SOAK_BIN:-$ROOT/target/release/loggytracy}"
PORT="${SOAK_PORT:-3153}"
...
if [ "${SOAK_SKIP_BUILD:-0}" != "1" ]; then
  ...
  [ -n "${SOAK_BIN:-}" ] || cargo build --manifest-path "$ROOT/Cargo.toml" --release \
    --bin loggytracy ${FEATURES:+--features "$FEATURES"}
fi
...
env LOGGYTRACY_LISTEN_ADDR="127.0.0.1:$PORT" \
    ...
    LOGGYTRACY_RETENTION_INTERVAL="$RETENTION_INTERVAL" \
    LOGGYTRACY_RETENTION_GRACE_PERIOD="$RETENTION_GRACE" \
    ${SOAK_SERVER_ENV:-} \
