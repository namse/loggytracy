...
#   M10_SERVER_ENV="LOGGYTRACY_MERGE_INTERVAL=3600s" ./scripts/run_memprof_local.sh no-merge
#   M10_SERVER_ENV="MALLOC_ARENA_MAX=1 MALLOC_TRIM_THRESHOLD_=131072" ./scripts/run_memprof_local.sh trimmed
#
# Query-shape weights are the harness's own `LOGGYTRACY_LOAD_QUERY_WEIGHT_*`
...
BIN="${M10_BIN:-$ROOT/target/release/loggytracy}"
PORT="${M10_PORT:-3151}"
UNIT="m10-$NAME"
...
if [ "${M10_SKIP_BUILD:-0}" != "1" ]; then
  ...
  [ -n "${M10_BIN:-}" ] || cargo build --manifest-path "$ROOT/Cargo.toml" --release \
    --bin loggytracy ${M10_FEATURES:+--features "$M10_FEATURES"}
fi
...
env LOGGYTRACY_LISTEN_ADDR="127.0.0.1:$PORT" \
    LOGGYTRACY_OTLP_GRPC_ADDR="127.0.0.1:$((PORT + 1000))" \
    LOGGYTRACY_DATA_DIR="$DATA" \
    ${M10_SERVER_ENV:-} \
...
(
  ...
  while [ -d "$CG" ]; do
    ...
    gg=$(echo "$m" | awk '
      ...
      $1=="loggytracy_pending_flush_bytes"{pf=$2}
      $1=="loggytracy_wal_backlog_bytes"{wb=$2}
...
LOGGYTRACY_LOAD_TARGET=loggytracy \
LOGGYTRACY_LOAD_PHASE=load \
LOGGYTRACY_LOAD_ADDR="127.0.0.1:$PORT" \
LOGGYTRACY_LOAD_CGROUP="$CG" \
LOGGYTRACY_LOAD_TIER=memprof \
LOGGYTRACY_LOAD_SEED="$SEED" \
LOGGYTRACY_LOAD_SECONDS="$SECONDS_CAP" \
LOGGYTRACY_LOAD_EVENTS="$EVENTS" \
LOGGYTRACY_LOAD_TARGET_EPS="$EPS" \
LOGGYTRACY_LOAD_CONNECTIONS="$CONNS" \
LOGGYTRACY_LOAD_QUERY_EPS="$QUERY_EPS" \
LOGGYTRACY_LOAD_OTLP_EPS=0 \
LOGGYTRACY_LOAD_RESULT_PATH="$OUT/load.json" \
