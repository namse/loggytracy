...
sequence: loggytracy gets metrics as its third signal — stored as
...
- **Parity is the bar; the budget is the axis.** loggytracy does not try to
...
- **Internal naming avoids `crate::metrics`.** `src/metrics.rs`
  ...
  `/loggytracy/api/v1/metrics/*` (first-party; no collision with the root
  `/metrics` scrape route), config knobs `LOGGYTRACY_MAX_ACTIVE_SERIES` /
  `LOGGYTRACY_MAX_METRIC_*`, `RemoteDomain::Metrics`, object-store prefix
  `metrics/`.
...
work. That is seven routes, all under `/loggytracy/api/v1/`, tenant from
`X-Scope-OrgID`, NDJSON out, `X-Loggytracy-Scanned-Rows` (samples decoded) /
`X-Loggytracy-Scanned-Bytes` headers, refusals as `application/json`
...
2. **Churn:** when a series-churn workload pushes active series past what the
   limit can index, loggytracy keeps ingesting known series and answering
...
churn with at least the same sample acceptance *and* beats loggytracy
...
sizes its index to the workload, loggytracy sizes the workload to its budget.
...
3. **New-series refusal.** When a tenant's index holds `max_active_series`
   ...
   `LOGGYTRACY_MAX_ACTIVE_SERIES`, and the idle-timeout horizon at which
...
- **A separate script, `compare/run_metrics.sh`**, on the `run.sh` skeleton
  ...
  to loggytracy `/v1/metrics` and VictoriaMetrics' OTLP endpoint
...
| knob | default |
|---|---|
| `LOGGYTRACY_MAX_METRIC_SAMPLES` (per request, post-decomposition) | 100 000 |
| `LOGGYTRACY_MAX_ACTIVE_SERIES` (per tenant) | 500 000 |
| `LOGGYTRACY_METRIC_SERIES_IDLE_TIMEOUT` | 10m |
| `LOGGYTRACY_MAX_CONCURRENT_METRIC_SCANS` | 8 |
| `LOGGYTRACY_MAX_METRIC_QUERY_RUNTIME` | 30s |
| `LOGGYTRACY_MAX_METRIC_RESTORE_RUNTIME` | 25s |
| `LOGGYTRACY_MAX_METRIC_SERIES_PER_QUERY` | 10 000 |
| `LOGGYTRACY_MAX_METRIC_POINTS_PER_QUERY` | 2 000 000 |
...
2. **The ruler's iron half.** The metric workload generator, the
   ...
   one-sided against VictoriaMetrics alone (the loggytracy column
...
8. **Bed completion and the gate.** The loggytracy drivers for
