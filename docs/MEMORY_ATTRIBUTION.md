...
1.2 M events at an offered 20 k eps with a ~5 qps read workload, **loggytracy is
...
`/metrics` grows the `loggytracy_memprof_*` families. `cargo test`
...
| `mp-nomerge` | memprof | `LOGGYTRACY_MERGE_INTERVAL=3600s` | **killed** | 41.5 s | 1656 |
...
ingest peak of **3936.2 MiB for loggytracy against 1294.3 MiB for Loki**, and
...
on a timer, and glibc's malloc does not. loggytracy's *live* working set at
...
live bytes, and loggytracy is the one that got killed at 2 GiB. It makes it a
...
`loggytracy_memtable_bytes` reports 217.9 MiB for the buffer still filling. The
...
`memprof live_bytes{ingest}` divided by `loggytracy_memtable_bytes`, measured
...
`loggytracy_part_sidecar_resident_bytes` reported 5.5 MiB where the allocator
