...
**Generated by `compare/run.sh` on 2026-08-02T09:50:58Z, from revision `649e622` (`master`). Do not edit
...
|---|---|---|---|
| build | `649e622` (`master`) | `release-3.3.x-23b5fc2` (`23b5fc2c`) | `v1.52.0` |
...
| **achieved eps** | 19616 | 19857 | 19894 |
| events accepted | 1201100 | 1200200 | 1200400 |
| elapsed s | 61.2 | 60.4 | 60.3 |
| stopped on | event_target | event_target | event_target |
| pushes accepted | 12011 | 12002 | 12004 |
| pushes throttled (429) | 0 | 0 | 0 |
| pushes failed | 0 | 0 | 0 |
| error rate | 0 | 0 | 0 |
| throttled rate | 0 | 0 | 0 |
| push service p50 ms | 9.86 | 1.12 | 0.27 |
| push service p95 ms | 19.3 | 3.45 | 0.51 |
| push service p99 ms | 99.0 | 5.39 | 1.42 |
| push response p50 ms | 11.2 | 2.28 | 1.81 |
| push response p95 ms | 71.4 | 4.86 | 2.29 |
| push response p99 ms | 189 | 12.0 | 3.13 |
| latency samples | 10011 | 10002 | 10004 |
| line bytes offered | 393250374 | 392963749 | 393026236 |
| wire bytes (OTLP protobuf) | 512304607 | 511928817 | 512011125 |
...
**loggytracy and Loki agreed on 145 of 168 queries on the strict
basis.** Every pair, per shape:

| shape | loggytracy = Loki (strict) | loggytracy = VictoriaLogs (reduced) | Loki = VictoriaLogs (reduced) |
|---|---|---|---|
| `label_only` | 1 / 24 | 24 / 24 | 1 / 24 |
| `line_filter` | 24 / 24 | 24 / 24 | 24 / 24 |
| `json_field` | 24 / 24 | 0 / 24 | 0 / 24 |
| `json_field_rare` | 24 / 24 | 16 / 24 | 16 / 24 |
| `metadata_rare` | 24 / 24 | 24 / 24 | 24 / 24 |
| `trace_window` | 24 / 24 | 24 / 24 | 24 / 24 |
| `rate` | 24 / 24 | 24 / 24 | 24 / 24 |

**loggytracy against victorialogs disagreed on 32 queries** (reduced basis): `json_field/api-gateway/w0` (1003 against null rows), `json_field/api-gateway/w1` (752 against null rows), `json_field/api-gateway/w2` (495 against null rows), `json_field/billing/w0` (795 against null rows), `json_field/billing/w1` (526 against null rows), `json_field/billing/w2` (794 against null rows), `json_field/checkout/w0` (1037 against null rows), `json_field/checkout/w1` (805 against null rows), `json_field/checkout/w2` (463 against null rows), `json_field/ingest/w0` (781 against null rows), and 22 more

**loki against victorialogs disagreed on 55 queries** (reduced basis): `json_field/api-gateway/w0` (1003 against null rows), `json_field/api-gateway/w1` (752 against null rows), `json_field/api-gateway/w2` (495 against null rows), `json_field/billing/w0` (795 against null rows), `json_field/billing/w1` (526 against null rows), `json_field/billing/w2` (794 against null rows), `json_field/checkout/w0` (1037 against null rows), `json_field/checkout/w1` (805 against null rows), `json_field/checkout/w2` (463 against null rows), `json_field/ingest/w0` (781 against null rows), and 45 more

**23 queries disagreed.** This is a correctness finding and it is reported before any timing conclusion is drawn from the same run. The last column counts label names only loggytracy had against label names only Loki had.

| query | loggytracy rows | Loki rows | loggytracy digest | Loki digest | labels only one side had |
|---|---|---|---|---|---|
| `label_only/api-gateway/w0` | 6250 | null | `0ce46c18588ded50` | `null` | 8 / 0 |
| `label_only/api-gateway/w1` | 6250 | null | `53f0965b787e0379` | `null` | 8 / 0 |
| `label_only/api-gateway/w2` | 6250 | null | `deead7fa2da84912` | `null` | 8 / 0 |
| `label_only/billing/w0` | 6250 | null | `843c91372fc409ca` | `null` | 8 / 0 |
| `label_only/billing/w1` | 6250 | null | `c712371491c603bc` | `null` | 8 / 0 |
| `label_only/billing/w2` | 6250 | null | `25d4027dc5f3bf9c` | `null` | 8 / 0 |
| `label_only/checkout/w0` | 6250 | null | `054e80a6d172840f` | `null` | 8 / 0 |
| `label_only/checkout/w1` | 6250 | null | `cf4d75c5181ed3ac` | `null` | 8 / 0 |
| `label_only/checkout/w2` | 6250 | null | `2010b9b53341589c` | `null` | 8 / 0 |
| `label_only/ingest/w0` | 6250 | null | `93d32dc99fe6b13f` | `null` | 8 / 0 |
| `label_only/ingest/w2` | 6250 | null | `8114ecadbddbcc40` | `null` | 8 / 0 |
| `label_only/notifier/w0` | 6250 | null | `0eb283ac436e2e90` | `null` | 8 / 0 |
| `label_only/notifier/w1` | 6250 | null | `72e468a778c301fb` | `null` | 8 / 0 |
| `label_only/notifier/w2` | 6250 | null | `1fb73c051f2d50b4` | `null` | 8 / 0 |
| `label_only/scheduler/w0` | 6250 | null | `fe491a3cb09f5ebf` | `null` | 8 / 0 |
| `label_only/scheduler/w1` | 6250 | null | `26df56404582c452` | `null` | 8 / 0 |
| `label_only/scheduler/w2` | 6250 | null | `7ed6ecb8698fea72` | `null` | 8 / 0 |
| `label_only/search/w0` | 6250 | null | `314524282c0f1c4f` | `null` | 8 / 0 |
| `label_only/search/w1` | 6250 | null | `87c10382b1906e78` | `null` | 8 / 0 |
| `label_only/search/w2` | 6250 | null | `3774d996be6ffeb3` | `null` | 8 / 0 |
...
* **23 queries** — `label_only/api-gateway/w0`, `label_only/api-gateway/w1`, `label_only/api-gateway/w2` and 20 more:
    * only loggytracy: `stream:cloud_region`, `stream:deployment_environment`, `stream:k8s_cluster_name`, `stream:k8s_container_name`, `stream:k8s_namespace_name`, `stream:pod_ip`, `stream:service_name`, `stream:trace_id`
    * only Loki: none


The declared exemption, as it applied to this run:

* **Loki** returned `stream:detected_level` on 121 answers, dropped from the digest by declaration.
...
|---|---|---|---|---|---|---|---|---|---|
| `label_only` | cold | 52.2 | null | 29.9 | 73.7 | null | 36.1 | **withheld (23/24 disagree)** | **1.74x** |
| `label_only` | warm | 49.9 | 176 | 26.8 | 54.4 | null | 31.7 | **withheld (23/24 disagree)** | **1.86x** |
| `line_filter` | cold | 19.4 | 23.5 | 8.93 | 23.4 | 24.9 | 10.4 | **0.83x** | **2.17x** |
| `line_filter` | warm | 19.3 | 23.8 | 7.96 | 23.6 | 27.5 | 11.3 | **0.81x** | **2.43x** |
| `json_field` | cold | 30.0 | 51.8 | null | 35.3 | 69.0 | null | **0.58x** | **withheld (24/24 disagree)** |
| `json_field` | warm | 29.9 | 51.9 | null | 35.8 | 67.6 | null | **0.58x** | **withheld (24/24 disagree)** |
| `json_field_rare` | cold | 2.55 | 135 | 79.3 | 2.83 | 137 | null | **0.02x** | **withheld (8/24 disagree)** |
| `json_field_rare` | warm | 2.59 | 135 | 80.6 | 2.88 | 138 | 85.6 | **0.02x** | **withheld (8/24 disagree)** |
| `metadata_rare` | cold | 2.20 | 78.8 | 1.71 | 2.30 | 81.0 | 2.08 | **0.03x** | **1.28x** |
| `metadata_rare` | warm | 2.18 | 78.7 | 1.60 | 2.30 | 81.3 | 3.11 | **0.03x** | **1.36x** |
| `trace_window` | cold | 2.11 | 31.9 | 1.44 | 2.21 | 34.1 | 1.84 | **0.07x** | **1.47x** |
| `trace_window` | warm | 2.12 | 31.8 | 1.52 | 2.35 | 34.0 | 2.62 | **0.07x** | **1.40x** |
| `rate` | cold | 3.05 | 11.2 | 0.41 | 4.02 | 11.5 | 0.49 | **0.27x** | **7.45x** |
| `rate` | warm | 3.09 | 10.7 | 0.42 | 4.17 | 12.2 | 0.58 | **0.29x** | **7.42x** |
...
|---|---|---|---|---|---|---|---|---|---|
| `label_only` | 20000 | 52.2 | null | 29.9 | **withheld (23/24 disagree)** | **1.74x** | 849392 | 6537 | 150000 |
| `label_only` | 100 | 16.7 | 8.41 | 15.1 | **withheld (23/24 disagree)** | **1.11x** | 296396 | 13686 | 2400 |
| `line_filter` | 20000 | 19.4 | 23.5 | 8.93 | **0.83x** | **2.17x** | 849392 | 157304 | 10208 |
| `line_filter` | 100 | 16.5 | 10.3 | 12.6 | **1.60x** | **1.31x** | 490079 | 85550 | 2400 |
| `json_field` | 20000 | 30.0 | 51.8 | null | **0.58x** | **withheld (24/24 disagree)** | 849392 | 156988 | 17661 |
| `json_field` | 100 | 20.6 | 14.6 | null | **1.41x** | **withheld (24/24 disagree)** | 849392 | 79279 | 2400 |
| `json_field_rare` | 20000 | 2.55 | 135 | 79.3 | **0.02x** | **withheld (8/24 disagree)** | 245760 | 1582288 | 24 |
| `json_field_rare` | 100 | 2.55 | 135 | 79.0 | **0.02x** | **withheld (8/24 disagree)** | 245760 | 1582288 | 24 |
| `metadata_rare` | 20000 | 2.20 | 78.8 | 1.71 | **0.03x** | **1.28x** | 245760 | 1582288 | 24 |
| `metadata_rare` | 100 | 2.28 | 79.4 | 1.56 | **0.03x** | **1.46x** | 245760 | 1582288 | 24 |
| `trace_window` | 20000 | 2.11 | 31.9 | 1.44 | **0.07x** | **1.47x** | 147456 | 574392 | 24 |
| `trace_window` | 100 | 2.12 | 32.0 | 1.50 | **0.07x** | **1.41x** | 147456 | 574392 | 24 |
| `rate` | 20000 | 3.05 | 11.2 | 0.41 | **0.27x** | **7.45x** | 849392 | 146551 | 112 |
| `rate` | 100 | 3.13 | 10.6 | 0.40 | **0.29x** | **7.75x** | 849392 | 146551 | 112 |
...
| `memory.peak` during ingest | 2048.1 MiB | 2048.1 MiB | 573.2 MiB |
| **anonymous peak during ingest** | 1423.1 MiB | 1115.3 MiB | 383.7 MiB |
| `memory.peak` during queries | 131.2 MiB | 332.2 MiB | 286.7 MiB |
...
| | loggytracy | Loki | VictoriaLogs |
|---|---|---|---|
| line bytes ingested | 442280203 | 441993578 | 442056065 |
| total on disk, settled | 616.7 MiB | 117.7 MiB | 53.8 MiB |
| of which write-ahead log | 549.0 MiB | 0.0 MiB | -0.0 MiB |
| settled data (total minus WAL) | 67.6 MiB | 117.7 MiB | 53.8 MiB |
| **total per GB ingested** | 1394 MiB / GB | 266 MiB / GB | 122 MiB / GB |
| **settled data per GB ingested** | 153 MiB / GB | 266 MiB / GB | 122 MiB / GB |
| total after the query phase | 616.7 MiB | 117.7 MiB | 53.8 MiB |

Breakdown:

* loggytracy — `/data/journal.ckpt:8,/data/journal.wal:575702910,/data/parts:70910697,/data/traces:4096`
* Loki — `/data/chunks:123289807,/data/compactor:4096,/data/rules:4096,/data/rules-temp:4096,/data/tsdb-shipper-active:107561,/data/tsdb-shipper-cache:24576,/data/wal:8192`
* VictoriaLogs — `/data/flock.lock:0,/data/partitions:56408565`
...
* against Loki, cold: loggytracy is 35.90x faster (2.2 ms against 78.8 ms)
* against Loki, warm: loggytracy is 36.09x faster (2.2 ms against 78.7 ms)
* against VictoriaLogs, cold: loggytracy is 1.28x slower (2.2 ms against 1.7 ms)
* against VictoriaLogs, warm: loggytracy is 1.36x slower (2.2 ms against 1.6 ms)
* ingest: loggytracy achieved 19616 eps against Loki's 19857 and VictoriaLogs' 19894
* disk: loggytracy's settled data is 0.57x Loki's and 1.26x VictoriaLogs' (write-ahead logs excluded on every side)

The previous claim's shape stays measured beside it: `json_field` cold is loggytracy is 1.73x faster (30.0 ms against 51.8 ms)
against Loki and **withheld** — the json_field answers disagree with VictoriaLogs against VictoriaLogs. That claim was measured, lost, and
...
* **The ingest runs are sequential, and loggytracy went first.** A later run starts with a warmer page cache and a machine that has just finished doing work. Running them concurrently would remove that and introduce a worse problem — each system's throughput would depend on the others' — so this is a chosen bias rather than an overlooked one, and its direction favours the systems that ran later (19616 / 19857 / 19894 achieved eps in run order).
...
* **23 queries errored on loki** and are excluded from its percentiles, so those percentiles describe the queries that succeeded rather than the workload that was offered.

* **32 queries errored on victorialogs** and are excluded from its percentiles, so those percentiles describe the queries that succeeded rather than the workload that was offered.
...
```diff
--- /dev/fd/63	2026-08-02 16:23:05.465740404 +0900
+++ /dev/fd/62	2026-08-02 16:23:05.465740404 +0900
@@ -7 +7 @@
-  http_listen_address: ""
...
```
[2m2026-08-02T07:22:47.259369Z[0m [32m INFO[0m [2mloggytracy::config[0m[2m:[0m configured peak materialized memory, excluding trace scans, the memtable and allocator retention [3mpeak_materialized_bytes[0m[2m=[0m5368709120 [3mconcurrent_query_scans[0m[2m=[0m8 [3mmax_query_memory_bytes[0m[2m=[0m536870912 [3mmerge_max_memory_bytes[0m[2m=[0m1073741824 [3mmerge_max_input_bytes[0m[2m=[0m536870912 [3mdetected_memory_limit_bytes[0m[2m=[0m2147483648 [3mdetected_memory_source[0m[2m=[0m"cgroup v2 memory.max"
[2m2026-08-02T07:22:47.259520Z[0m [32m INFO[0m [2mloggytracy::startup[0m[2m:[0m journal recovery complete [3mcheckpoint[0m[2m=[0m0 [3mreplay_end[0m[2m=[0m0
[2m2026-08-02T07:22:47.279396Z[0m [33m WARN[0m [2mloggytracy::startup[0m[2m:[0m no retention is configured: neither LOGGYTRACY_RETENTION_PERIOD nor LOGGYTRACY_TENANT_POLICY_TOKEN is set, so the object store grows without bound
[2m2026-08-02T07:22:47.279405Z[0m [33m WARN[0m [2mloggytracy::startup[0m[2m:[0m bound to a non-loopback address: this process has no TLS and no authentication, and X-Scope-OrgID is trusted without proof. Keep it inside a trust boundary [3maddr[0m[2m=[0m0.0.0.0:4317
[2m2026-08-02T07:22:47.279443Z[0m [32m INFO[0m [2mloggytracy::startup[0m[2m:[0m loggytracy listening [3maddr[0m[2m=[0m0.0.0.0:3100
[2m2026-08-02T07:22:47.279462Z[0m [33m WARN[0m [2mloggytracy::startup[0m[2m:[0m bound to a non-loopback address: this process has no TLS and no authentication, and X-Scope-OrgID is trusted without proof. Keep it inside a trust boundary [3maddr[0m[2m=[0m0.0.0.0:3100
```
