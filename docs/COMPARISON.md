...
**Generated by `compare/run.sh` on 2026-08-02T10:21:44Z, from revision `c005b06` (`master`). Do not edit
...
|---|---|---|---|
| build | `c005b06` (`master`) | `release-3.3.x-23b5fc2` (`23b5fc2c`) | `v1.52.0` |
...
| **achieved eps** | 19621 | 19863 | 19932 |
| events accepted | 1201100 | 1200600 | 1200300 |
| elapsed s | 61.2 | 60.4 | 60.2 |
| stopped on | event_target | event_target | event_target |
| pushes accepted | 12011 | 12006 | 12003 |
| pushes throttled (429) | 0 | 0 | 0 |
| pushes failed | 0 | 0 | 0 |
| error rate | 0 | 0 | 0 |
| throttled rate | 0 | 0 | 0 |
| push service p50 ms | 2.18 | 1.12 | 0.26 |
| push service p95 ms | 2.57 | 3.46 | 0.45 |
| push service p99 ms | 7.40 | 5.40 | 1.07 |
| push response p50 ms | 3.27 | 2.08 | 1.33 |
| push response p95 ms | 3.89 | 4.56 | 1.80 |
| push response p99 ms | 8.59 | 8.59 | 2.25 |
| latency samples | 10011 | 10006 | 10003 |
| line bytes offered | 393250374 | 393087620 | 392997605 |
| wire bytes (OTLP protobuf) | 512304607 | 512092310 | 511972576 |
...
| `json_field_rare` | 24 / 24 | 8 / 24 | 8 / 24 |
| `metadata_rare` | 24 / 24 | 24 / 24 | 24 / 24 |
| `trace_window` | 24 / 24 | 24 / 24 | 24 / 24 |
| `rate` | 24 / 24 | 24 / 24 | 24 / 24 |

**loggytracy against victorialogs disagreed on 16 queries** (reduced basis): `json_field_rare/api-gateway/w0` (1 against 0 rows), `json_field_rare/api-gateway/w2` (1 against 0 rows), `json_field_rare/billing/w0` (1 against 0 rows), `json_field_rare/billing/w2` (1 against 0 rows), `json_field_rare/checkout/w0` (1 against 0 rows), `json_field_rare/checkout/w2` (1 against 0 rows), `json_field_rare/ingest/w0` (1 against 0 rows), `json_field_rare/ingest/w2` (1 against 0 rows), `json_field_rare/notifier/w0` (1 against 0 rows), `json_field_rare/notifier/w2` (1 against 0 rows), and 6 more

**loki against victorialogs disagreed on 16 queries** (reduced basis): `json_field_rare/api-gateway/w0` (1 against 0 rows), `json_field_rare/api-gateway/w2` (1 against 0 rows), `json_field_rare/billing/w0` (1 against 0 rows), `json_field_rare/billing/w2` (1 against 0 rows), `json_field_rare/checkout/w0` (1 against 0 rows), `json_field_rare/checkout/w2` (1 against 0 rows), `json_field_rare/ingest/w0` (1 against 0 rows), `json_field_rare/ingest/w2` (1 against 0 rows), `json_field_rare/notifier/w0` (1 against 0 rows), `json_field_rare/notifier/w2` (1 against 0 rows), and 6 more
...
|---|---|---|---|---|---|---|---|---|---|
| `label_only` | cold | 50.2 | 174 | 29.3 | 63.7 | 181 | 33.4 | **0.29x** | **1.71x** |
| `label_only` | warm | 49.0 | 179 | 25.1 | 54.4 | 187 | 29.4 | **0.27x** | **1.95x** |
| `line_filter` | cold | 18.6 | 23.9 | 7.89 | 21.8 | 25.6 | 9.21 | **0.78x** | **2.35x** |
| `line_filter` | warm | 18.2 | 23.6 | 7.22 | 22.0 | 26.4 | 9.91 | **0.77x** | **2.52x** |
| `json_field` | cold | 29.5 | 52.1 | 13.4 | 33.2 | 69.4 | 14.5 | **0.57x** | **2.20x** |
| `json_field` | warm | 28.8 | 52.1 | 12.7 | 33.2 | 68.7 | 15.2 | **0.55x** | **2.28x** |
| `json_field_rare` | cold | 3.13 | 136 | 63.1 | 3.29 | 139 | 67.1 | **0.02x** | **withheld (16/24 disagree)** |
| `json_field_rare` | warm | 3.06 | 135 | 62.9 | 3.32 | 138 | 66.0 | **0.02x** | **withheld (16/24 disagree)** |
| `metadata_rare` | cold | 2.73 | 78.7 | 1.42 | 2.82 | 81.6 | 1.78 | **0.03x** | **1.92x** |
| `metadata_rare` | warm | 2.51 | 79.0 | 1.43 | 2.74 | 81.2 | 1.75 | **0.03x** | **1.76x** |
| `trace_window` | cold | 2.58 | 31.8 | 1.38 | 2.71 | 33.5 | 1.54 | **0.08x** | **1.87x** |
| `trace_window` | warm | 2.42 | 31.9 | 1.35 | 2.57 | 34.0 | 1.62 | **0.08x** | **1.79x** |
| `rate` | cold | 2.96 | 10.7 | 0.43 | 3.86 | 11.7 | 0.56 | **0.28x** | **6.95x** |
| `rate` | warm | 2.78 | 10.7 | 0.42 | 3.65 | 12.1 | 0.64 | **0.26x** | **6.55x** |
...
|---|---|---|---|---|---|---|---|---|---|
| `label_only` | 20000 | 50.2 | 174 | 29.3 | **0.29x** | **1.71x** | 780784 | 156516 | 150000 |
| `label_only` | 100 | 14.8 | 8.37 | 13.6 | **1.77x** | **1.09x** | 252006 | 13686 | 2400 |
| `line_filter` | 20000 | 18.6 | 23.9 | 7.89 | **0.78x** | **2.35x** | 780784 | 157304 | 10208 |
| `line_filter` | 100 | 15.7 | 10.5 | 12.5 | **1.49x** | **1.25x** | 449738 | 85550 | 2400 |
| `json_field` | 20000 | 29.5 | 52.1 | 13.4 | **0.57x** | **2.20x** | 780784 | 156988 | 17661 |
| `json_field` | 100 | 19.0 | 14.5 | 15.0 | **1.31x** | **1.27x** | 774044 | 79279 | 2400 |
| `json_field_rare` | 20000 | 3.13 | 136 | 63.1 | **0.02x** | **withheld (16/24 disagree)** | 344064 | 1582288 | 24 |
| `json_field_rare` | 100 | 3.02 | 135 | 62.2 | **0.02x** | **withheld (16/24 disagree)** | 344064 | 1582288 | 24 |
| `metadata_rare` | 20000 | 2.73 | 78.7 | 1.42 | **0.03x** | **1.92x** | 344064 | 1582288 | 24 |
| `metadata_rare` | 100 | 2.49 | 78.9 | 1.43 | **0.03x** | **1.74x** | 344064 | 1582288 | 24 |
| `trace_window` | 20000 | 2.58 | 31.8 | 1.38 | **0.08x** | **1.87x** | 180224 | 574392 | 24 |
| `trace_window` | 100 | 2.40 | 32.3 | 1.35 | **0.07x** | **1.77x** | 180224 | 574392 | 24 |
| `rate` | 20000 | 2.96 | 10.7 | 0.43 | **0.28x** | **6.95x** | 780784 | 146551 | 112 |
| `rate` | 100 | 2.76 | 10.6 | 0.41 | **0.26x** | **6.76x** | 780784 | 146551 | 112 |
...
| `memory.peak` during ingest | 2048.0 MiB | 2048.1 MiB | 628.7 MiB |
| **anonymous peak during ingest** | 1391.1 MiB | 1088.8 MiB | 420.8 MiB |
| `memory.peak` during queries | 129.9 MiB | 347.2 MiB | 271.9 MiB |
...
| | loggytracy | Loki | VictoriaLogs |
|---|---|---|---|
| line bytes ingested | 442280203 | 442117449 | 442027434 |
| total on disk, settled | 620.3 MiB | 117.8 MiB | 55.4 MiB |
| of which write-ahead log | 549.0 MiB | 0.0 MiB | -0.0 MiB |
| settled data (total minus WAL) | 71.2 MiB | 117.7 MiB | 55.4 MiB |
| **total per GB ingested** | 1402 MiB / GB | 266 MiB / GB | 125 MiB / GB |
| **settled data per GB ingested** | 161 MiB / GB | 266 MiB / GB | 125 MiB / GB |
| total after the query phase | 620.3 MiB | 117.8 MiB | 55.4 MiB |

Breakdown:

* loggytracy — `/data/journal.ckpt:8,/data/journal.wal:575702910,/data/parts:74681584,/data/traces:4096`
* Loki — `/data/chunks:123350721,/data/compactor:4096,/data/rules:4096,/data/rules-temp:4096,/data/tsdb-shipper-active:75333,/data/tsdb-shipper-cache:24576,/data/wal:8192`
* VictoriaLogs — `/data/flock.lock:0,/data/partitions:58038789`
...
* against Loki, cold: loggytracy is 28.87x faster (2.7 ms against 78.7 ms)
* against Loki, warm: loggytracy is 31.43x faster (2.5 ms against 79.0 ms)
* against VictoriaLogs, cold: loggytracy is 1.92x slower (2.7 ms against 1.4 ms)
* against VictoriaLogs, warm: loggytracy is 1.76x slower (2.5 ms against 1.4 ms)
* ingest: loggytracy achieved 19621 eps against Loki's 19863 and VictoriaLogs' 19932
* disk: loggytracy's settled data is 0.60x Loki's and 1.29x VictoriaLogs' (write-ahead logs excluded on every side)

The previous claim's shape stays measured beside it: `json_field` cold is loggytracy is 1.77x faster (29.5 ms against 52.1 ms)
against Loki and loggytracy is 2.20x slower (29.5 ms against 13.4 ms) against VictoriaLogs. That claim was measured, lost, and
...
* **The ingest runs are sequential, and loggytracy went first.** A later run starts with a warmer page cache and a machine that has just finished doing work. Running them concurrently would remove that and introduce a worse problem — each system's throughput would depend on the others' — so this is a chosen bias rather than an overlooked one, and its direction favours the systems that ran later (19621 / 19863 / 19932 achieved eps in run order).
...
```diff
--- /dev/fd/63	2026-08-02 19:11:30.046350124 +0900
+++ /dev/fd/62	2026-08-02 19:11:30.046350124 +0900
@@ -7 +7 @@
-  http_listen_address: ""
...
```
[2m2026-08-02T10:11:11.866141Z[0m [32m INFO[0m [2mloggytracy::config[0m[2m:[0m configured peak materialized memory, excluding trace scans, the memtable and allocator retention [3mpeak_materialized_bytes[0m[2m=[0m5368709120 [3mconcurrent_query_scans[0m[2m=[0m8 [3mmax_query_memory_bytes[0m[2m=[0m536870912 [3mmerge_max_memory_bytes[0m[2m=[0m1073741824 [3mmerge_max_input_bytes[0m[2m=[0m536870912 [3mdetected_memory_limit_bytes[0m[2m=[0m2147483648 [3mdetected_memory_source[0m[2m=[0m"cgroup v2 memory.max"
[2m2026-08-02T10:11:11.866362Z[0m [32m INFO[0m [2mloggytracy::startup[0m[2m:[0m journal recovery complete [3mcheckpoint[0m[2m=[0m0 [3mreplay_end[0m[2m=[0m0
[2m2026-08-02T10:11:11.871666Z[0m [33m WARN[0m [2mloggytracy::startup[0m[2m:[0m no retention is configured: neither LOGGYTRACY_RETENTION_PERIOD nor LOGGYTRACY_TENANT_POLICY_TOKEN is set, so the object store grows without bound
[2m2026-08-02T10:11:11.871672Z[0m [33m WARN[0m [2mloggytracy::startup[0m[2m:[0m bound to a non-loopback address: this process has no TLS and no authentication, and X-Scope-OrgID is trusted without proof. Keep it inside a trust boundary [3maddr[0m[2m=[0m0.0.0.0:4317
[2m2026-08-02T10:11:11.871718Z[0m [32m INFO[0m [2mloggytracy::startup[0m[2m:[0m loggytracy listening [3maddr[0m[2m=[0m0.0.0.0:3100
[2m2026-08-02T10:11:11.871722Z[0m [33m WARN[0m [2mloggytracy::startup[0m[2m:[0m bound to a non-loopback address: this process has no TLS and no authentication, and X-Scope-OrgID is trusted without proof. Keep it inside a trust boundary [3maddr[0m[2m=[0m0.0.0.0:3100
```
