...
## 검증 환경 — S3 대상 테스트를 하지 않는다

실 클라우드에는 비용 때문에, 로컬 MinIO에는 **`object_store` 크레이트를 신뢰하기 때문에** 붙이지
않는다. S3 프로토콜 구현이 맞는지는 그 크레이트의 책임이고 자기 테스트 스위트가 있다. 부하 검증은
인프로세스 지연·장애 주입만으로 한다.

검증 범위의 원칙은 **우리 코드만 테스트한다**는 것이다. S3 wire 프로토콜·조건부 PUT 시맨스·
multipart가 맞게 구현됐는지는 `object_store` 크레이트의 책임이므로 여기서 재검증하지 않는다.
우리 몫은 그 아래 한 칸 — **이 바이너리가 이 설정으로 만든 스토어가 실제로 CAS를 하는가** — 이고,
그건 부하 테스트가 아니라 부팅 프리플라이트로 확인한다(아래).

따라서 부하 런은 우리 루프의 행동(flush 전진, backlog 유계, backpressure, RSS 안정, 시작 시간)을
보며, 그 항목 중 오브젝트 스토어 백엔드에 의존하는 것은 없다. 다음은 그럼에도 남는 위험이다.

- **지연 분포의 꼬리** — 루프백은 1 ms 미만이고 실 S3는 p99가 수백 ms~수 초다. 지연 주입 래퍼는
  백엔드를 가리지 않으므로 MinIO 위에 씌워 실 wire와 꼬리를 동시에 얻을 수 있고, 부하 스크립트가
  기본으로 그렇게 돌린다. 다만 주입값은 실측이 아니라 가정이고, 주입이 `object_store` 클라이언트
  위에서 일어나므로 그 재시도 층은 검증되지 않는다. **남은 위험 중 가장 큰 축이다.**
- **스로틀링** — S3는 prefix당 요청률을 제한하고 MinIO는 하지 않는다. 다만 기본 설정에서 manifest는
  0.2 PUT/s, 전체도 10 PUT/s 수준이라 한도와 네 자릿수 차이가 나므로 실제 위험은 작다.
...
**CAS 프리플라이트.** 기동 시 `ObjectStorage::verify_conditional_put`이 프로브 객체로 *거절되어야
할 쓰기가 거절되는지*를 확인하고, 아니면 기동을 거부한다. 긍정 경로는 아무것도 증명하지 못한다 —
빈 prefix의 첫 manifest 쓰기는 조건이 지켜지든 말든 성공하기 때문이다. 이 검사가 배포 대상 스토어
자기 자신에 대해 돌기 때문에, 로컬에서는 답할 수 없던 "이 제공자에서 CAS가 되는가"가 여기서
해결된다. `file://`은 CAS를 의도적으로 포기한 개발용 백엔드이므로 건너뛴다.
