...
const TARGETS: [&str; 3] = ["signy", "loki", "victorialogs"];
...
const PAIRS: [(&str, &str, Basis); 3] = [
    ("signy", "loki", Basis::Strict),
    ("signy", "victorialogs", Basis::Reduced),
...
| shape | limit | signy cold p50 | Loki cold p50 | VictoriaLogs cold p50 | lt / Loki | lt / VL | signy lines read | Loki lines read | rows returned |\n\
|---|---|---|---|---|---|---|---|---|---|\n",
    );
    ...
    for shape in SHAPES {
        for run in runs {
            ...
            let lt = read_json(dir, &format!("matrix_signy{suffix}.json"));
    ...
    page.push_str(
        "\nMilliseconds, and a ratio above `1.00x` means signy took longer. \
...
fn header(page: &mut String, bed: &Value) {
    page.push_str(&format!(
        r#"# signy against Loki and VictoriaLogs
...
> signy answers `{{...}} | field="value"` over structured metadata — the
...
That builds the signy image, brings all three systems up under
...
| volume | `signy-data` | `loki-data` | `victorialogs-data` |
...
**Why signy runs with no object store.** Loki's filesystem backend keeps
exactly one durable copy of a chunk on local disk, and so does VictoriaLogs.
Pointing signy at a `file://` object store would make it keep a local part
...
makes the three comparable. It is also the configuration in which signy's
object-store request counters are all zero, which is half of why that axis is
deferred below.

Everything else on the signy side is the engine's own default. The
...
"#,
    ...
    page.push_str("| | signy | Loki | VictoriaLogs |\n|---|---|---|---|\n");
...
system where signy is unthrottled, which is a misconfiguration reported as
...
signy has resident part sidecars, VictoriaLogs has its own caches, and the
...
"#,
        num(&matrix["signy"]["config"]["verify"]["repeats"]),
        num(&matrix["signy"]["verify"]["rows"]),
        num(&matrix["signy"]["verify"]["streams"]),
    ));

    page.push_str(
        "| shape | pass | signy p50 | Loki p50 | VictoriaLogs p50 | \
signy p95 | Loki p95 | VictoriaLogs p95 | lt / Loki (p50) | lt / VL (p50) |\n\
|---|---|---|---|---|---|---|---|---|---|\n",
    );
    for shape in SHAPES {
        for pass in ["cold_ms", "warm_ms"] {
            let lt = &matrix["signy"]["matrix"]["shapes"][shape][pass];
    ...
    page.push_str(
        "\nMilliseconds. A ratio above `1.00x` means signy took longer. A `null` \
...
is in the artifacts.\n\n",
    ...
    for shape in SHAPES {
        page.push_str(&format!(
            "* `{shape}` — `{}`\n",
            matrix["signy"]["matrix"]["shapes"][shape]["expression_example"]
                .as_str()
                .unwrap_or("?"),
        ));
    }
...
one series per `trace_id` on Loki and one series per stream on signy —
...
the spread between them on the signy side is what the parser stage and
...
fn row_equality(page: &mut String, matrix: &BTreeMap<&str, Value>, agreements: &[PairAgreement]) {
    let lt = indexed_answers(matrix, "signy");
...
store the line — signy and Loki — each response is reduced to an
...
signy and Loki answer under the promoted `service_name` — and a metric
...
signy returned them in the third element of each `values` tuple. That was
...
signy's step-from-`start` produce the same instants. And an *unaggregated*
metric query still differs in series identity — Loki promotes a row's structured
metadata and extracted fields into it, signy groups by stream labels and by
...
**signy and Loki agreed on {agreed} of {total} queries on the strict
basis.** Every pair, per shape:

| shape | signy = Loki (strict) | signy = VictoriaLogs (reduced) | Loki = VictoriaLogs (reduced) |
|---|---|---|---|
"#
    ));
    for shape in SHAPES {
    ...
    if mismatches.is_empty() {
        page.push_str("\nNo strict-basis mismatches between signy and Loki.\n");
    } else {
        page.push_str(&format!(
...
column counts label names only signy had against label names only Loki \
had.\n\n\
| query | signy rows | Loki rows | signy digest | Loki digest | labels only one side had |\n\
|---|---|---|---|---|---|\n{}\n\n",
            mismatches.len(),
            mismatches
                .iter()
        ...
        if off_by_one > 0 {
            page.push_str(&format!(
                r#"In {off_by_one} of the {} disagreements signy returned **exactly one row
...
signy's `query_range` treats `end` as **inclusive**, Loki treats it as
**exclusive**, and both include `start`. Loki's is the contract signy's
endpoint claims to implement, so this is a Loki-compatibility defect on the
signy side, found by this check and recorded in `todo.md`. It is invisible
...
structured-metadata object, and `metric:` one in a series' identity:\n\n",
    ...
    let all_answers: [(&str, &BTreeMap<String, Value>); 3] =
        [("signy", &lt), ("Loki", &lk), ("VictoriaLogs", &vl)];
...
| limit | signy survived / OOM | Loki survived / OOM | VictoriaLogs survived / OOM |
|---|---|---|---|
"#,
        );
...
the fairer one.**
signy compacts its write-ahead log only when an object store is
...
charging signy for the bed's decision — the same defect as a rigged win,
...
signy flushes every five seconds at its default and needed no equivalent.

"#,
        disk["signy"]["breakdown"].as_str().unwrap_or(""),
...
signy's `signy_object_store_operations_total` counts calls into the
...
signy's storage path end to end (restores, the local cache, the manifest
...
What is already known and does not need this bed: signy's counts are
...
fn verdict(
    ...
    let describe = |name: &str, pass: &str, other: &str, other_label: &str| -> String {
        if !pair_agrees("signy", other, name) {
            return format!("**withheld** — the {name} answers disagree with {other_label}");
        }
        match (shape(name, pass, "signy"), shape(name, pass, other)) {
            (Some(lt), Some(them)) if them > 0.0 => {
                let factor = lt / them;
                if factor < 0.9 {
                    format!(
                        "signy is {:.2}x faster ({lt:.1} ms against {them:.1} ms)",
                        1.0 / factor
                    )
                } else if factor > 1.1 {
                    format!("signy is {factor:.2}x slower ({lt:.1} ms against {them:.1} ms)")
                } else {
    ...
    let disk_line = {
        ...
        let lt = data_of("signy");
        let lk = data_of("loki");
        let vl = data_of("victorialogs");
        if lk > 0.0 && vl > 0.0 {
            format!(
                "signy's settled data is {:.2}x Loki's and {:.2}x VictoriaLogs' \
(write-ahead logs excluded on every side)",
    ...
    let against = |other: &str, threshold: f64, pass: &str| -> Option<bool> {
        if !pair_agrees("signy", other, "metadata_rare") {
            return None;
        }
        shape("metadata_rare", pass, "signy")
...
systems genuinely differ here by design: signy indexes structured metadata
...
* ingest: signy achieved {:.0} eps against Loki's {:.0} and VictoriaLogs' {:.0}
* disk: {}
...
fn distrust(
    ...
    items.push(
        "**Loki answers this window from memory and signy answers it from disk.** \
...
as well as the store. signy has no equivalent: after a restart its rows \
...
signy was OOM-killed at the lower limit and Loki was not, so the query, \
disk and memory columns are all taken at a limit signy needed and Loki \
...
limit until the losing side fits.",
    ...
    let wal_share = {
        let total = f64_of(&bed["disk_bytes"]["signy"]["settled"]).unwrap_or(0.0);
        if total > 0.0 {
            wal_bytes(bed, "signy") / total
        } else {
            0.0
        }
    };
    if wal_share > 0.25 {
        items.push(format!(
            "**{:.0}% of signy's disk number is a write-ahead log the bed's own \
configuration stops it from compacting.** `flush.rs:219` compacts the WAL only \
when an object store is configured, and this bed runs signy local-only so \
...
the one an operator running local-only actually gets.",
    ...
    items.push(format!(
        "**The ingest runs are sequential, and signy went first.** A later run \
...
systems that ran later ({} / {} / {} achieved eps in run order).",
        num(&load["signy"]["ingest"]["achieved_eps"]),
...
fn configuration(page: &mut String, dir: &Path) {
    ...
    let env = read_text(dir, "signy_env.txt");
    let startup = read_text(dir, "signy_startup.log");
    page.push_str(&format!(
...
(`loki_config.yaml`, `loki_config_defaults.yaml`, `signy_startup.log`).
...
`reject_old_samples_max_age` (1w, the same window as signy's
...
### signy: the container's whole configuration surface
...
const METRIC_TARGETS: [&str; 2] = ["signy", "victoriametrics"];
...
fn metrics_header(page: &mut String, bed: &Value) {
    page.push_str(&format!(
        r#"# signy against VictoriaMetrics
...
fn metrics_verdict(
    ...
    for shape in steady_shapes {
        ...
        let ours = f64_of(&matrix["signy"]["matrix"]["shapes"][shape]["cold_ms"]["p50_ms"]);
        let theirs =
    ...
    let ours_survived = bed["churn_survival"]["signy"]["survived"] == Value::Bool(true);
    let steady_acceptance =
        f64_of(&load["signy"]["load"]["phases"]["steady"]["acceptance"]).unwrap_or(0.0);
    ...
    // as one it never met.
    let refused = named_refusals(&load["signy"]);
    ...
    for shape in METRIC_SHAPES {
        ...
        let ours = &matrix["signy"]["matrix"]["shapes"][shape]["cold_ms"]["p50_ms"];
