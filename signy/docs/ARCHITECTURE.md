...
### Object-store settings
 ...
 - `SIGNY_OBJECT_STORE_URL`: Format `s3://bucket/prefix`. Development and tests may use single-process `file:///absolute/path`. `file://` manifest updates use in-process serialization and atomic rename and do not provide multi-writer CAS. Unset means local-only mode.
