# signy against Loki and VictoriaLogs

**Generated by `compare/run.sh` on 2026-08-12T12:55:03Z, from revision `a1aa5d1` (`master`). Do not edit
...
[`artifacts/m9-10x/`](artifacts/m9-10x/), which the same run copied out of
...
|---|---|---|---|
| build | `a1aa5d1` (`master`) | `release-3.3.x-23b5fc2` (`23b5fc2c`) | `v1.52.0` |
...
| **achieved eps** | 19918 | 19860 | 19818 |
| events accepted | 1201100 | 1200500 | 1200700 |
| elapsed s | 60.3 | 60.4 | 60.6 |
| stopped on | event_target | event_target | event_target |
| pushes accepted | 12011 | 12005 | 12007 |
| pushes throttled (429) | 0 | 0 | 0 |
| pushes failed | 0 | 0 | 0 |
| error rate | 0 | 0 | 0 |
| throttled rate | 0 | 0 | 0 |
| push service p50 ms | 10.1 | 1.13 | 0.26 |
| push service p95 ms | 23.0 | 3.49 | 0.49 |
| push service p99 ms | 90.3 | 5.41 | 2.58 |
| push response p50 ms | 11.6 | 2.08 | 0.96 |
| push response p95 ms | 91.9 | 4.70 | 1.44 |
| push response p99 ms | 253 | 11.3 | 3.16 |
| latency samples | 10011 | 10005 | 10007 |
| line bytes offered | 393250374 | 393057634 | 393119777 |
| wire bytes (OTLP protobuf) | 512304607 | 512052424 | 512134376 |
...
**signy and Loki agreed on 168 of 168 queries on the strict
...
* **Loki** returned `stream:__stream_shard__` on 58 answers, `stream:detected_level` on 144 answers, dropped from the digest by declaration.
...
| shape | pass | signy p50 | Loki p50 | VictoriaLogs p50 | signy p95 | Loki p95 | VictoriaLogs p95 | lt / Loki (p50) | lt / VL (p50) |
|---|---|---|---|---|---|---|---|---|---|
| `label_only` | cold | 68.4 | 593 | 126 | 84.5 | 620 | 146 | **0.12x** | **0.54x** |
| `label_only` | warm | 75.0 | 588 | 124 | 90.3 | 621 | 153 | **0.13x** | **0.60x** |
| `line_filter` | cold | 53.7 | 210 | 43.2 | 60.8 | 219 | 55.9 | **0.26x** | **1.25x** |
| `line_filter` | warm | 55.2 | 211 | 42.6 | 63.7 | 221 | 57.3 | **0.26x** | **1.29x** |
| `json_field` | cold | 102 | 495 | 85.2 | 135 | 531 | 117 | **0.21x** | **1.20x** |
| `json_field` | warm | 103 | 492 | 83.1 | 141 | 543 | 118 | **0.21x** | **1.23x** |
| `json_field_rare` | cold | 0.44 | 974 | 191 | 15.6 | 989 | 212 | **0.00x** | **0.00x** |
| `json_field_rare` | warm | 0.45 | 976 | 190 | 12.2 | 998 | 214 | **0.00x** | **0.00x** |
| `metadata_rare` | cold | 0.38 | 561 | 5.99 | 3.13 | 565 | 8.85 | **0.00x** | **0.06x** |
| `metadata_rare` | warm | 0.37 | 561 | 5.80 | 0.41 | 567 | 8.64 | **0.00x** | **0.06x** |
| `trace_window` | cold | 0.93 | 53.7 | 1.82 | 2.30 | 72.8 | 4.00 | **0.02x** | **0.51x** |
| `trace_window` | warm | 0.29 | 53.1 | 1.68 | 1.80 | 75.6 | 3.26 | **0.01x** | **0.18x** |
| `rate` | cold | 4.53 | 84.4 | 1.26 | 5.04 | 89.8 | 1.70 | **0.05x** | **3.59x** |
| `rate` | warm | 4.62 | 83.9 | 1.23 | 5.13 | 91.3 | 1.71 | **0.06x** | **3.74x** |

Milliseconds. A ratio above `1.00x` means signy took longer. A `null` percentile is one the sample count could not support — `stats.rs::min_samples_for` refuses `p99` under 101 samples and `p95` under 21, and `null` is not a pass. p99 is in the artifacts.
...
|---|---|---|---|---|---|---|---|---|---|
| `label_only` | 20000 | 68.4 | 593 | 126 | **0.12x** | **0.54x** | 1200313 | 561600 | 480000 |
| `label_only` | 100 | 13.4 | 28.0 | 27.2 | **0.48x** | **0.49x** | 184325 | 130092 | 2400 |
| `line_filter` | 20000 | 53.7 | 210 | 43.2 | **0.26x** | **1.25x** | 1995616 | 1562984 | 102351 |
| `line_filter` | 100 | 14.0 | 19.3 | 35.0 | **0.73x** | **0.40x** | 300129 | 135581 | 2400 |
| `json_field` | 20000 | 102 | 495 | 85.2 | **0.21x** | **1.20x** | 1975136 | 1562031 | 177129 |
| `json_field` | 100 | 13.0 | 25.9 | 41.6 | **0.50x** | **0.31x** | 992096 | 132843 | 2400 |
| `json_field_rare` | 20000 | 0.44 | 974 | 191 | **0.00x** | **0.00x** | 29696 | 12764768 | 224 |
| `json_field_rare` | 100 | 0.44 | 976 | 169 | **0.00x** | **0.00x** | 29696 | 12764768 | 224 |
| `metadata_rare` | 20000 | 0.38 | 561 | 5.99 | **0.00x** | **0.06x** | 29696 | 12764768 | 224 |
| `metadata_rare` | 100 | 0.39 | 561 | 4.14 | **0.00x** | **0.09x** | 29696 | 12764768 | 224 |
| `trace_window` | 20000 | 0.93 | 53.7 | 1.82 | **0.02x** | **0.51x** | 35840 | 814226 | 26 |
| `trace_window` | 100 | 0.95 | 52.6 | 1.63 | **0.02x** | **0.58x** | 35840 | 814226 | 26 |
| `rate` | 20000 | 4.53 | 84.4 | 1.26 | **0.05x** | **3.59x** | 1995306 | 1548711 | 1192 |
| `rate` | 100 | 4.39 | 85.3 | 1.22 | **0.05x** | **3.59x** | 1995306 | 1548711 | 1192 |

Milliseconds, and a ratio above `1.00x` means signy took longer. "Lines read" is each system's own `data.stats.summary.totalLinesProcessed`, summed over the shape's answers — what the engine had to touch to produce the answer, which is the quantity pruning and early termination exist to reduce. It is reported rather than gated: the systems count it in their own terms, and VictoriaLogs does not report it at all. The agreement gate on the ratio columns is the primary run's; each limit's own agreement is in its artifact.
...
| `memory.peak` during ingest | 1117.6 MiB | 2048.2 MiB | 1027.6 MiB |
| **anonymous peak during ingest** | 684.1 MiB | 1126.3 MiB | 439.1 MiB |
| `memory.peak` during queries | 605.9 MiB | 710.6 MiB | 438.8 MiB |
...
| | signy | Loki | VictoriaLogs |
|---|---|---|---|
| line bytes ingested | 883721929 | 883529189 | 883591332 |
| total on disk, settled | 289.4 MiB | 267.5 MiB | 166.3 MiB |
| of which write-ahead log | 17.7 MiB | 9.5 MiB | -0.0 MiB |
| settled data (total minus WAL) | 271.7 MiB | 258.1 MiB | 166.3 MiB |
| **total per GB ingested** | 327 MiB / GB | 303 MiB / GB | 188 MiB / GB |
| **settled data per GB ingested** | 307 MiB / GB | 292 MiB / GB | 188 MiB / GB |
| total after the query phase | 289.4 MiB | 258.1 MiB | 166.3 MiB |

Breakdown:

* signy — `/data/journal.ckpt:8,/data/journal.wal:18579819,/data/parts:284882809,/data/traces:4096`
* Loki — `/data/chunks:270432474,/data/compactor:4096,/data/rules:4096,/data/rules-temp:4096,/data/tsdb-shipper-active:124922,/data/tsdb-shipper-cache:24576,/data/wal:9936896`
* VictoriaLogs — `/data/flock.lock:0,/data/partitions:174363000`
...
* `metadata_rare`, cold: **holds** — materially faster than Loki and not materially worse than VictoriaLogs
* `metadata_rare`, warm: **holds** — materially faster than Loki and not materially worse than VictoriaLogs
* against Loki, cold: signy is 1470.47x faster (0.4 ms against 560.8 ms)
* against Loki, warm: signy is 1511.42x faster (0.4 ms against 561.0 ms)
* against VictoriaLogs, cold: signy is 15.71x faster (0.4 ms against 6.0 ms)
* against VictoriaLogs, warm: signy is 15.61x faster (0.4 ms against 5.8 ms)
* ingest: signy achieved 19918 eps against Loki's 19860 and VictoriaLogs' 19818
* disk: signy's settled data is 1.05x Loki's and 1.63x VictoriaLogs' (write-ahead logs excluded on every side)

The previous claim's shape stays measured beside it: `json_field` cold is signy is 4.85x faster (102.1 ms against 495.1 ms)
against Loki and signy is 1.20x slower (102.1 ms against 85.2 ms) against VictoriaLogs. That claim was measured, lost, and
...
* **Loki answers this window from memory and signy answers it from disk.** Loki keeps flushed chunks resident and `query_ingesters_within` is three hours by default, so every query in the matrix hits the ingester's in-memory chunks as well as the store. signy has no equivalent: after a restart its rows are in Parquet parts and it reads them. Both are each system's default behaviour and neither was changed, but it means the query columns are not 'two engines reading the same medium'. A comparison over a window older than Loki's ingester retention would answer this, and it needs a run measured in hours rather than minutes.

* **The ingest runs are sequential, and signy went first.** A later run starts with a warmer page cache and a machine that has just finished doing work. Running them concurrently would remove that and introduce a worse problem — each system's throughput would depend on the others' — so this is a chosen bias rather than an overlooked one, and its direction favours the systems that ran later (19918 / 19860 / 19818 achieved eps in run order).
...
```diff
--- /dev/fd/63	2026-08-12 21:31:24.820931298 +0900
+++ /dev/fd/62	2026-08-12 21:31:24.820931298 +0900
@@ -7 +7 @@
-  http_listen_address: ""
...
```
SIGNY_OTLP_GRPC_ADDR=0.0.0.0:4317
SIGNY_DATA_DIR=/var/lib/signy
...
```
[2m2026-08-12T12:31:06.609331Z[0m [32m INFO[0m [2msigny[0m[2m:[0m glibc malloc tuning (SIGNY_MALLOC_TUNING=off to disable) [3mapplied[0m[2m=[0mfalse
[2m2026-08-12T12:31:06.609437Z[0m [32m INFO[0m [2msigny::config[0m[2m:[0m configured peak materialized memory, excluding trace scans and allocator retention [3mmemory_budget_bytes[0m[2m=[0m1288490188 [3mmemory_budget_source[0m[2m=[0m60% of 2147483648 (cgroup v2 memory.max) [3mpeak_materialized_bytes[0m[2m=[0m644245094 [3mquery_memory_budget_bytes[0m[2m=[0m322122547 [3mrow_group_cache_max_bytes[0m[2m=[0m161061273 [3msidecar_cache_max_bytes[0m[2m=[0m128849018 [3mmax_memtable_bytes[0m[2m=[0m128849018 [3mconcurrent_query_scans[0m[2m=[0m8 [3mmax_query_memory_bytes[0m[2m=[0m322122547 [3mmerge_max_memory_bytes[0m[2m=[0m322122547 [3mmerge_max_input_bytes[0m[2m=[0m161061273 [3mflush_chunk_bytes[0m[2m=[0m33554432 [3mdetected_memory_limit_bytes[0m[2m=[0m2147483648 [3mdetected_memory_source[0m[2m=[0m"cgroup v2 memory.max"
[2m2026-08-12T12:31:06.610808Z[0m [32m INFO[0m [2msigny::startup[0m[2m:[0m journal recovery complete [3mcheckpoint[0m[2m=[0m0 [3mreplay_end[0m[2m=[0m0
[2m2026-08-12T12:31:06.631014Z[0m [33m WARN[0m [2msigny::startup[0m[2m:[0m no retention is configured: neither SIGNY_RETENTION_PERIOD nor SIGNY_TENANT_POLICY_TOKEN is set, so the object store grows without bound
[2m2026-08-12T12:31:06.631031Z[0m [33m WARN[0m [2msigny::startup[0m[2m:[0m bound to a non-loopback address: this process has no TLS and no authentication, and X-Scope-OrgID is trusted without proof. Keep it inside a trust boundary [3maddr[0m[2m=[0m0.0.0.0:4317
[2m2026-08-12T12:31:06.631079Z[0m [32m INFO[0m [2msigny::startup[0m[2m:[0m signy listening [3maddr[0m[2m=[0m0.0.0.0:3100
[2m2026-08-12T12:31:06.631102Z[0m [33m WARN[0m [2msigny::startup[0m[2m:[0m bound to a non-loopback address: this process has no TLS and no authentication, and X-Scope-OrgID is trusted without proof. Keep it inside a trust boundary [3maddr[0m[2m=[0m0.0.0.0:3100
```
